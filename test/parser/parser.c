...
static ast_node_list_t* create_ast_node_list(src_file_t* file){
...
ast_node_t* parse_expr_until(src_file_t* file, token_t* until){
    ...
    for (uint8_t i = 0; i < sizeof(operators) / sizeof(operators[0]); i++){
        ...
        switch (op.type) {
            ...
            case OPT_BINARY:
                ...
                break;
            case OPT_TERNARY:
                throw_code_issue(*file, COMP_ERR_UNIMPLEMENTED, // TODO: ternary
    ...
    if((next = expect(TOKEN_TYPE_LPAREN))){
        ...
        newnode->func_call.args = create_ast_node_list(file);

        node = newnode;

        token_t* rparen = get_next_uspa(TOKEN_TYPE_RPAREN, until);
...
ast_node_t* parse_stmt(UNUSED src_file_t* file){
    ...
    if(is_qualifier(next) || is_basetype(next)){
        ...
        node->block.stmts = create_ast_node_list(file);
...
ast_node_t* parse_programism(src_file_t* file){
    ...
    if(next->type == TOKEN_TYPE_SEMI){
        ...
        node->func_decl.args = create_ast_node_list(file);
...
ast_node_t* parse_program(src_file_t* file){
    ...
    toret->program.programisms = create_ast_node_list(file);
...
void print_storage_type(src_file_t* file,storage_type_t type,size_t indent){
...
void print_ast_node(src_file_t* file,ast_node_t* node,size_t indent){
    char* indent_str = malloc(indent+1);
    ...
    switch(node->type){
        case AST_TYPE_PROGRAM:
            ...
            for(size_t i = 0;i < node->program.programisms->count;i++){
                print_ast_node(file,node->program.programisms->nodes[i], indent + INDENT_WIDTH);
            }
            break;
        ...
        case AST_TYPE_DECL:
            ...
            print_storage_type(file,node->decl.type, indent + INDENT_WIDTH);
            ...
            free(indent_str);
            if(node->decl.starting_value){
                print_ast_node(file,node->decl.starting_value, indent + INDENT_WIDTH*2);
            }
            break;
        ...
        case AST_TYPE_FUNC_DECL:
            ...
            print_storage_type(file,node->decl.type, indent + INDENT_WIDTH*2);
            printf("%s  |->Name: \"%s\"\n",indent_str,node->func_decl.name);
            printf("%s  |->Arguments: \n",indent_str);
            for(size_t i = 0;i < node->func_decl.args->count;i++){
                print_ast_node(file,node->func_decl.args->nodes[i], indent + INDENT_WIDTH*2);
            }
            if(node->func_decl.body){
                printf("%s  \\->Body: \n",indent_str);
                print_ast_node(file,node->func_decl.body, indent + INDENT_WIDTH*2);
            }
            free(indent_str); // no need for it after this
            break;
        ...
        case AST_TYPE_FUNC_CALL:
            ...
            print_ast_node(file,node->func_call.fp, indent + INDENT_WIDTH*2);
            printf("%s  \\->Arguments: \n",indent_str);
            free(indent_str); // no need for it after this
            for(size_t i = 0;i < node->func_call.args->count;i++){
                print_ast_node(file,node->func_call.args->nodes[i], indent + INDENT_WIDTH*2);
            }
            break;
        ...
        case AST_TYPE_BLOCK:
            ...
            for(size_t i = 0;i < node->block.stmts->count;i++){
                print_ast_node(file,node->block.stmts->nodes[i], indent + INDENT_WIDTH);
            }
            break;
        
        case AST_TYPE_EVAL:
            printf("%sEval:\n",indent_str);
            print_ast_node(file,node->eval.expr, indent + INDENT_WIDTH);
            break;
        ...
        case AST_TYPE_IF:
            ...
            printf("%s  |->Condition: \n",indent_str);
            print_ast_node(file,node->if_stmt.cond, indent + INDENT_WIDTH*2);
            printf("%s  \\->Body: \n",indent_str);
            print_ast_node(file,node->if_stmt.body, indent + INDENT_WIDTH*2);
            free(indent_str);
            break;
        ...
        case AST_TYPE_RETURN:
            ...
            if(node->return_stmt.val){
                printf("%s  \\->Value: \n",indent_str);
                print_ast_node(file,node->return_stmt.val, indent + INDENT_WIDTH*2);
            } 
            free(indent_str);
            break;
        ...
        case AST_TYPE_ASS:
            printf("%sAssignment:\n",indent_str);
            print_ast_node(file,node->binary.left, indent + INDENT_WIDTH);
            print_ast_node(file,node->binary.right, indent + INDENT_WIDTH);
            free(indent_str);
            break;
        ...
        case AST_TYPE_TERNARY_COND:
            printf("%sDivision:\n",indent_str);
            print_ast_node(file,node->ternary.cond, indent + INDENT_WIDTH);
            print_ast_node(file,node->ternary.val_true, indent + INDENT_WIDTH);
            print_ast_node(file,node->ternary.val_false, indent + INDENT_WIDTH);
            free(indent_str);
            break;
        ...
        case AST_TYPE_UN_MINUS:
            printf("%sUnary minus:\n",indent_str);
            print_ast_node(file,node->unary.expr, indent + INDENT_WIDTH);
            free(indent_str);
            break;
